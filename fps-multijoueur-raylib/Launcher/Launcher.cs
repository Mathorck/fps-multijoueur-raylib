...
public class Launcher
{
    ...
    private static float BTN_HEBERGER_WIDTH = CalculateButtonWidth(TEXT_HEBERGER), BTN_HEBERGER_HEIGHT = (int)(FONT_SIZE * 1.5),
                         BTN_PLAY_WIDTH = CalculateButtonWidth(TEXT_PLAY), BTN_PLAY_HEIGHT = FONT_SIZE * 2;
    ...
    /// </summary>
    private static Rectangle btnPlay = CreateButton(TEXT_PLAY);
    private static Rectangle btnHeberger = CreateButton(TEXT_HEBERGER, btnPlay.X - BTN_HEBERGER_WIDTH);
    private static Rectangle recBack = new(0, 0, GetScreenWidth() / 15, GetScreenHeight() / 15);
    #endregion

    private static List<ServerObject> servers = new List<ServerObject>();
    ...
    private static void Play(ref bool close)
    {
        ...
        foreach (ServerObject s in servers)
        {
            s.Draw();
    ...
    private static void Heberger(ref string ip)
    {
        ...
        {
            DrawText("Appuyez sur RETOUR pour supprimer des caractères...", GetScreenWidth() / 2 - MeasureText("Appuyez sur RETOUR pour supprimer des caractères...", 20) / 2, GetScreenHeight() / 2 - 300, 20, Color.Gray);
        }
    ...
    private static List<ServerObject> GetServers()
    {
        servers.Clear();
        ...
        using (MySqlConnection conn = new(connectionString))
        {
            ...
            using (MySqlDataReader reader = requete.ExecuteReader())
            {
                ...
                {
                    servers.Add(new(reader.GetInt32(0), reader.GetString(1), reader.GetString(2), reader.GetInt32(3), nbPassage));
                    nbPassage++;
                }
            }
            conn.Close();
    ...
    private static float CalculateButtonWidth(string text) => MeasureText(text, FONT_SIZE) + 2 * FONT_SIZE;
    ...
    private static Rectangle CreateButton(string text, float xPosition = -1) => new(
