...
namespace DeadOpsArcade3D.Launcher
{
    ...
    public class Login
    {
        ...
        const string TEXT_USERNAME = "Pseudo", TEXT_PASSWORD = "Mot de passe", TEXT_SIGNIN = "CONNECTER", TEXT_SINGUP = "Créer", TEXT_TOP = "Bienvenue !", TEXT_REMEMBER_ME = "Remember me";
        const int MAX_INPUT_CHARS_USERNAME = 25, MAX_INPUT_CHARS_PASSWORD = 25;
        ...
        public static void Start()
        {
            ...
            int INPUT_RECTANGLE_WIDTH = (int)(GetScreenWidth() / 1.5), INPUT_RECTANGLE_HEIGHT = GetScreenHeight() / 10, SPACE_BETWEEN = GetScreenHeight() / 30, FONT_SIZE = GetScreenHeight() / 30;

            string username = "", password = "";
            bool close = false, isRememberMe = false;

            // Définition des rectangles pour les champs de saisie
            Rectangle recTextBoxUsername = new(GetScreenWidth() / 2 - INPUT_RECTANGLE_WIDTH / 2, GetScreenHeight() / 3, INPUT_RECTANGLE_WIDTH, INPUT_RECTANGLE_HEIGHT);
            Rectangle recTextBoxPassword = new(GetScreenWidth() / 2 - INPUT_RECTANGLE_WIDTH / 2, recTextBoxUsername.Y + INPUT_RECTANGLE_HEIGHT + FONT_SIZE, INPUT_RECTANGLE_WIDTH, INPUT_RECTANGLE_HEIGHT);
            Rectangle recSignin = new(GetScreenWidth() / 2 - INPUT_RECTANGLE_WIDTH / 2, (int)(recTextBoxPassword.Y + INPUT_RECTANGLE_HEIGHT + FONT_SIZE), INPUT_RECTANGLE_WIDTH, INPUT_RECTANGLE_HEIGHT);
            Rectangle recSingup = new(GetScreenWidth() / 2 - INPUT_RECTANGLE_WIDTH / 2, (int)(recSignin.Y + INPUT_RECTANGLE_HEIGHT + FONT_SIZE), INPUT_RECTANGLE_WIDTH, INPUT_RECTANGLE_HEIGHT);
            Rectangle recRememberMe = new(GetScreenWidth() / 2 - INPUT_RECTANGLE_WIDTH / 2, (int)(recSingup.Y + INPUT_RECTANGLE_HEIGHT + FONT_SIZE), INPUT_RECTANGLE_HEIGHT, INPUT_RECTANGLE_HEIGHT);

            // Initialisation des champs de saisie et des boutons
            InputButton textBoxUsername = new(recTextBoxUsername, TEXT_USERNAME, MAX_INPUT_CHARS_USERNAME, Color.LightGray, Color.Black, Color.Black, Color.LightGray, Color.Black, Color.Black, false);
            InputButton textBoxPassword = new(recTextBoxPassword, TEXT_PASSWORD, MAX_INPUT_CHARS_PASSWORD, Color.LightGray, Color.Black, Color.Black, Color.LightGray, Color.Black, Color.Black, true);
            ...
            while (!IsKeyPressed(KeyboardKey.Enter) && !WindowShouldClose() && !close && !IsLoggedIn)
            {
                ...
                HandleUserInput(ref erreur, ref isHoverButton, ref isHoverInput, key, textBoxUsername, textBoxPassword, Signin, Singup, recRememberMe, ref isRememberMe);
        ...
        /// </summary>
        private static void LoadRememberedLoginInfo(ref InputButton textBoxUsername, ref InputButton textBoxPassword, ref bool isRememberMe)
        {
        ...
        /// </summary>
        private static void HandleUserInput(ref string erreur, ref bool isHoverButton, ref bool isHoverInput, int key, InputButton textBoxUsername, InputButton textBoxPassword, Boutton Signin, Boutton Singup, Rectangle recRememberMe, ref bool isRememberMe)
        {
        ...
        /// </summary>
        private static void DrawUIElements(List<InputButton> inputButtons, List<Boutton> Bouttons, Rectangle recRememberMe, string TEXT_REMEMBER_ME, string erreur, bool isRememberMe, int FONT_SIZE)
        {
            ...
            DrawText(TEXT_REMEMBER_ME, (int)(recRememberMe.X + recRememberMe.Width + FONT_SIZE), (int)(recRememberMe.Y + recRememberMe.Height / 2 - FONT_SIZE / 2), FONT_SIZE, Color.Black);
            DrawText(erreur, (int)(recRememberMe.X + recRememberMe.Width + MeasureText(TEXT_REMEMBER_ME, FONT_SIZE) + 2 * FONT_SIZE), (int)(recRememberMe.Y + recRememberMe.Width / 2 - FONT_SIZE / 2), FONT_SIZE, Color.Red);
        }
        ...
        private static string verifyInputConnect(string username, string password)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = new($"SELECT pseudo FROM user WHERE pseudo = \"{username}\" AND MotDePasse = \"{password}\"", conn);
                using (MySqlDataReader reader = cmd.ExecuteReader())
                {
                    if (reader.HasRows)
                        return "";
        ...
        private static bool Create(string Username, string Password, bool rememberme)
        {
            try
            {
                ...
                {
                    conn.Open();
                    MySqlCommand cmd = new($"INSERT INTO user (Pseudo, MotDePasse) VALUE (\"{Username}\", \"{Password}\")", conn);
                    cmd.ExecuteNonQuery();
        ...
        private static bool Connect(string Username, string Password, bool rememberme)
        {
            try
            {
                ...
                {
                    conn.Open();
                    MySqlCommand cmd = new($"SELECT Id, Pseudo, MotDePasse FROM user WHERE Pseudo = \"{Username}\" AND MotDePasse = \"{Password}\";", conn);
                    using (MySqlDataAdapter adapter = new MySqlDataAdapter(cmd))
                    {
                        ...
                        if (dt.Rows.Count == 1)
                        {
                            ...
                            Player = new((int)dt.Rows[0]["Id"], (string)dt.Rows[0]["Pseudo"]);
                            IsLoggedIn = true;
                            CloseWindow();
                            Launcher.Init();
                            return true;
